...
from hplc.io import parse_agilent_d_data_file, is_agilent_d_chromatogram
from hplc.models import Chromatogram, Signal2D

example_agilent_filename = "tests/example_chromatograms/example_agilent_d_chromatogram.D"
desired_filename = example_agilent_filename.split("/")[-1]
example_agilent_filename_modified_1 = "tests/example_chromatograms/example_agilent_d_chromatogram"
example_agilent_filename_modified_2 = "tests/example_chromatograms/example_agilent_d_chromatogram.D/"
...
def agilent_chromatogram():
    return parse_agilent_d_data_file(example_agilent_filename)
...
def test_is_agilent_d_chromatogram_detects_agilent_d_filetype():
    assert is_agilent_d_chromatogram(example_agilent_filename) is True
    assert is_agilent_d_chromatogram(example_agilent_filename_modified_1) is True
    assert is_agilent_d_chromatogram(example_agilent_filename_modified_2) is True

def test_is_agilent_d_chromatogram_rejects_non_agilent_d_filetype():
    assert is_agilent_d_chromatogram(example_shimadzu_filename) is False
    assert is_agilent_d_chromatogram(example_ez_chrom_filename) is False
    assert is_agilent_d_chromatogram(example_agilent_aia_filename) is False
